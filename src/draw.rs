...
use crate::{
    camera::Camera, instance::Instance2D, render_context::RenderContext
};
...
pub fn render<T: Camera + Sized>(render_context: &mut RenderContext<T>, queue: &Queue) {
    ...
    let (draw_circle, draw_arrow) = {
        ...
        let draw_arrow = |p: Vector2<f32>, rotation: f32, scale: Vector2<f32>, color: Vector4<f32>| {
    ...
    draw_arrow(vec2(0.0, 0.0), PI/2.0, vec2(1.0, 1.0), vec4(0.0, 1.0, 1.0, 1.0));
